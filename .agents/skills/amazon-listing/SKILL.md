...
把"创建 listing / 拉取产品信息 / 绑定 A+"固化为可复用流程。脚本在 `scripts/`,从**本仓根目录**运行。

## 前置条件(每次先确认)
**三个功能现在统一只依赖一件事:中间层 multi-channel-api 在跑。**
1. **中间层 multi-channel-api 必须在跑**(create / pull / A+ 全走它的 HTTP)。需带:
   - Amazon 路由 + `VALIDATION_PREVIEW` 透传(`products.py` 的 create 支持 `mode`);
   - **A+ 端点**(`/amazon/aplus/*`,在 `app/platforms/amazon/aplus.py` + `amazon.py`)。
   - 地址 env `AMAZON_MCA_URL`(默认 `http://localhost:8000`;我之前的实例在 `:8001`)。没起会报 NETWORK。
   - 起法:`cd <multi-channel-api> && python -m uvicorn app.main:app --port 8000`
2. **COS 已配**(本仓 `.env` 的 `COS_*`)——create_listing / aplus create 的本地图上传要用(外部对象存储,非要起的服务)。
   - 不依赖本仓 :8888 web 应用。
...
```bash
python .agents/skills/amazon-listing/scripts/create_listing.py validate payload.json   # 零写入校验
python .agents/skills/amazon-listing/scripts/create_listing.py create   payload.json   # 真建/更新
```
...
```bash
python .agents/skills/amazon-listing/scripts/edit_listing.py <SKU> set item_name="新标题" style=Cartoon finish_type=Matte
python .agents/skills/amazon-listing/scripts/edit_listing.py <SKU> price 12.99 [--list 16.99]   # 改价
python .agents/skills/amazon-listing/scripts/edit_listing.py <SKU> stock 50                      # 改库存(0=草稿,>0=可售)
python .agents/skills/amazon-listing/scripts/edit_listing.py <SKU> patches patches.json          # 任意 patch,完全控制
```
...
```bash
python .agents/skills/amazon-listing/scripts/pull_product.py <SKU> [<SKU> ...]   # 摘要(状态/ASIN/标题/属性/图数/价)
python .agents/skills/amazon-listing/scripts/pull_product.py <SKU> --json        # 完整 attributes
python .agents/skills/amazon-listing/scripts/pull_product.py --asin <ASIN> [<ASIN>..]   # 按 ASIN 查 catalog 摘要(标题/品牌/style/color/型号/类目/主图/排名);加 --json 看完整
```
...
```bash
python .agents/skills/amazon-listing/scripts/aplus.py list                                 # 列出所有 A+(名/状态/badge/key)
python .agents/skills/amazon-listing/scripts/aplus.py bind <KEY> <ASIN|SKU> [...]   # 关联 A+(默认提审;--no-submit 只关联;**传 SKU 自动解析当前 ASIN,防 AC-1022,推荐**)
python .agents/skills/amazon-listing/scripts/aplus.py create "<名称>" <ASIN> <img1.png> [img2...]  # 用图建标准A+→关联→提审
```
...
python list_products.py --store 店[@站点] [--all] [--csv f.csv]  # **拉全店产品**(SKU/ASIN/价/库存/BuyBox;赛狐数据,分钟级延迟)
```
...
## 相关记忆/文档(更细的坑)
`docs/amazon_custom_listing_design.md`、`docs/amazon_sticker_decal_attributes.example.json`,
以及记忆:amazon-field-facts / amazon-aplus-api / amazon-cdn / amazon-test-safety / amazon-variant-pattern。
